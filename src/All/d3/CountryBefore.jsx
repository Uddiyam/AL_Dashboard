...
import visit_data from "./전국방문_전기준.csv";
import search_data from "./전국검색_전기준.csv";

class ContryBefore extends React.Component {
  /*
    constructor(props) {
        super(props);
  ...
  barChart(check) {
    ...
    d3.csv(check).then(function (data) {
      ...
      // Add Y axis
      const y = d3.scaleBand().domain(groups).range([height, 0]).padding([0.2]);
