...
import input_data from "./강원도.csv";

function GangwonSearch() {
  ...
  useEffect(() => {
    const margin = { top: 30, right: 30, bottom: 70, left: 70 },
        ...
        d3.csv(input_data).then(function (data) {
            ...
            // Add Y axis
            y.domain([0, d3.max(data, d => +d['search'])]);
            ...
            svg.selectAll("rect")
                ...
                //.style("fill", function (d) { if (d.time == '코로나 후') { return "#A4A4A4" } else { return "#084B8A" } });    //코로나 후일때
        })
  }, []);
